...
{
    const std::set<tripoint_abs_omt> &existing_camps = get_player_character().camps;
    for( const tripoint_abs_omt &blah : points ) {
        if( existing_camps.count( blah ) > 0 ) {
            continue; // Already a camp there
        }
        // TODO: Check for camp that's too close??
        std::optional<basecamp *> bcp = overmap_buffer.find_camp( blah.xy() );
        if( bcp ) {
            continue; // Already a camp nearby, can't put down another one
        }
        get_map().add_camp( blah, "faction_camp", false );
        bcp = overmap_buffer.find_camp( blah.xy() );
        if( !bcp ) {
        ...
        const oter_type_str_id &keyvalue = ter( project_remain<coords::om>
                                                ( blah ).remainder_tripoint )->get_type_id();
        temp_camp->set_owner( camp_map[keyvalue].second );
        temp_camp->set_name( camp_map[keyvalue].first );
        temp_camp->define_camp( blah, "faction_base_bare_bones_NPC_camp_0", false );
    }
}
