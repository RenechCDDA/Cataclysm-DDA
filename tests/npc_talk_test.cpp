...
TEST_CASE( "npc_compare_int", "[npc_talk]" )
{
    ...
    int current_line = 0;
    // response 0
    CHECK( d.responses[ current_line++ ].text == "This is a math test response that increments by 1." );
    CHECK( d.responses[ current_line++ ].text ==
           "This is an math test response that increments by 2." );
    CHECK( d.responses[ current_line++ ].text == "PC strength is five." );
    CHECK( d.responses[ current_line++ ].text == "PC dexterity is six." );
    CHECK( d.responses[ current_line++ ].text == "PC intelligence is seven." );
    CHECK( d.responses[ current_line++ ].text == "PC perception is eight." );
    CHECK( d.responses[ current_line++ ].text == "NPC strength is nine." );
    CHECK( d.responses[ current_line++ ].text == "NPC dexterity is ten." );
    CHECK( d.responses[ current_line++ ].text == "NPC intelligence is eleven." );
    CHECK( d.responses[ current_line++ ].text == "NPC perception is twelve." );
    // response 10
    CHECK( d.responses[ current_line++ ].text == "PC Custom var is one." );
    CHECK( d.responses[ current_line++ ].text == "NPC Custom var is two." );
    CHECK( d.responses[ current_line++ ].text == "This is a u_var time test response for > 3_days." );
    CHECK( d.responses[ current_line++ ].text == "time_since_cataclysm > 3_days." );
    CHECK( d.responses[ current_line++ ].text == "time_since_cataclysm in days > 3" );
    CHECK( d.responses[ current_line++ ].text == "Allies equals 1" );
    CHECK( d.responses[ current_line++ ].text == "Cash equals 13" );
    CHECK( d.responses[ current_line++ ].text == "Owed amount equals 14" );
    CHECK( d.responses[ current_line++ ].text == "Driving skill more than or equal to 5" );
    // TODO: Reliably test the random number generator.
    CHECK( d.responses[ current_line++ ].text == "Temperature is 21." );
    // response 20
    CHECK( d.responses[ current_line++ ].text == "Windpower is 15." );
    CHECK( d.responses[ current_line++ ].text == "Humidity is 16." );
    CHECK( d.responses[ current_line++ ].text == "Pressure is 17." );
    CHECK( d.responses[ current_line++ ].text == "Pos_x is -1." );
    CHECK( d.responses[ current_line++ ].text == "Pos_y is -2." );
    CHECK( d.responses[ current_line++ ].text == "Pos_z is -3." );
    CHECK( d.responses[ current_line++ ].text == "Pain level is 21." );
    CHECK( d.responses[ current_line++ ].text == "Bionic power is 22." );
    CHECK( d.responses[ current_line++ ].text == "Bionic power max is 44." );
    CHECK( d.responses[ current_line++ ].text == "Bionic power is at 50%." );
    // response 30
    CHECK( d.responses[ current_line++ ].text == "Morale is 23." );
    CHECK( d.responses[ current_line++ ].text == "Focus is 24." );
    CHECK( d.responses[ current_line++ ].text == "Mana is 25." );
    CHECK( d.responses[ current_line++ ].text == "Mana max is 900." );
    CHECK( d.responses[ current_line++ ].text == "Mana is at 2%." );
    CHECK( d.responses[ current_line++ ].text == "Hunger is 26." );
    CHECK( d.responses[ current_line++ ].text == "Thirst is 27." );
    CHECK( d.responses[ current_line++ ].text == "Stored kcal is 118'169." );
    CHECK( d.responses[ current_line++ ].text == "Has 3 glass bottles." );
    CHECK( d.responses[ current_line++ ].text == "Highest spell level in school test_trait is 1." );
    // response 40
    CHECK( d.responses[ current_line++ ].text == "Spell level of Pew, Pew is 4." );
    CHECK( d.responses[ current_line++ ].text == "Spell level of highest spell is 12." );
    CHECK( d.responses[ current_line++ ].text == "Exp of Pew, Pew is 11006." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency learning is 50% out of 100%." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency learning done is 12 hours total." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency learning is 50% learnt." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency learning is 500 permille learnt." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency total learning time is 24 hours." );
    CHECK( d.responses[ current_line++ ].text == "Test Proficiency time lest to learn is 12h." );
